...
class Metric:
    ...
    def compute(self) -> dict[str, float]:
        if "metric_args" in TASK_ATTRS[self.task_name]:
            results = self.metric.compute(**TASK_ATTRS[self.task_name]["metric_args"])
        else:
...
def preprocess_for_classification(
    ...
    # print("predictions", logits.argmax(-1).tolist(), "references", labels.tolist())
