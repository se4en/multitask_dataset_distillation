...
class TrainConfig:
    ...
    max_grad_norm: Union[float, None] = 2.0
    val_interval: int = 1
    ...
    fp16: bool = False
    bf16: bool = False
    task_weigths: Dict[str, float] = None
        ...
        for i in range(self.config.epoch):
            ...
            with tqdm(
                ...
                for outer_step, batch_real in enumerate(pbar):
                    ...
                    # print(f"Number of traineble params {sum(p.numel() for n, p in model.named_parameters() if p.requires_grad)}")

                    # print("params", len(list(params.keys())))
                    # print("buffers", len(list(buffers.keys())))
