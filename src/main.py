...
def main(config: Config):
    ...
    mlflow.log_metrics({f"val_avg.{k}": v[0] for k, v in val_results.items()})
    mlflow.log_metrics({f"val_std.{k}": v[1] for k, v in val_results.items()})

    val_results = {k: f"{v[0]}±{v[1]}" for k, v in val_results.items()}
    logger.info(f"Final Val Results: {val_results}")
    save_path = os.path.join(config.base.save_dir, "val_results.json")
    json.dump(val_results, open(save_path, "w"))
    mlflow.log_artifact(save_path)

    if data_module.has_test():
        test_results = evaluator.evaluate(
            distilled_data, eval_loader=data_module.test_loader(), verbose=True
        )
        mlflow.log_metrics({f"test_avg.{k}": v[0] for k, v in test_results.items()})
        mlflow.log_metrics({f"test_std.{k}": v[1] for k, v in test_results.items()})

        test_results = {k: f"{v[0]}±{v[1]}" for k, v in test_results.items()}
        logger.info(f"Final Test Results: {test_results}")
        save_path = os.path.join(config.base.save_dir, "test_results.json")
